...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = pinnedMap.length
      ? [...new Set(pinnedMap.map(([, v]) => slash(resolve(PKG_DIR, v))))].filter((p) => existsSync(p))
      : srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p));
    entry = join(OUT, '.pkg-entry.mjs');
    // FORK: `export *` does NOT forward a module's default export. These
    // components are default exports (`export default function TrialPlayer`),
    // so without an explicit `export { default as <Name> }` they never land on
    // window.<GLOBAL> ([BUNDLE_EXPORT]). For each pinned Name whose file has no
    // matching NAMED export, alias its default to the Name; the `export *`
    // lines still carry genuine named exports (e.g. LeaderboardPanel).
    const lines = comps.map((p) => `export * from ${JSON.stringify(p)};`);
    for (const [name, rel] of pinnedMap) {
      const p = slash(resolve(PKG_DIR, rel));
      if (!existsSync(p)) continue;
      const named = new RegExp(
        `export\\s+(?:const|let|var|function|class)\\s+${name}\\b|export\\s*\\{[^}]*\\b${name}\\b`,
      ).test(readText(p));
      if (!named) lines.push(`export { default as ${name} } from ${JSON.stringify(p)};`);
    }
